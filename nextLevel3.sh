...
# --- Read config file if it exists (JSON format) ---
### === Logging & Helpers ===
...
run_diagnostic() {
  ...
  return $actual_exit_status                                       # Return the diagnostic command's actual exit code
}
...
# Corrected diag_brew function (fixed structural errors)
diag_launchagents() {
...
on_error() {
  ...
  log INFO "--- General System Info ---" || true # Ensure this log doesn't fails
...
# Check if the plist file exists before attempting to load it
...
# Check if the service is valid and not empty
# 3) Get Colima VM IP
...
# 3) Check if the Colima VM IP is valid
...
if $ENABLE_NORD; then # Use uppercase variable
  ...
  run_diagnostic "open -a NordVPN || true"
fi
...
# Check if the DNS is set to the Pi-hole IP or localhost
