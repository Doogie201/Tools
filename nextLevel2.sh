#!/usr/bin/env zsh
###############################################################################
# nextLevel.sh v2.1 (Corrected) – One-shot “genius-level” setup for M2 Pro Mac
###############################################################################
set -eEuo pipefail
...
if [[ -f $CONFIG ]]; then
  ...
  for v in INSTALL_PKGS INSTALL_CASKS INSTALL_OLLAMA INSTALL_PIHOLE INSTALL_DOH ENABLE_YUBI ENABLE_NORD; do
    [[ "${(P)v}" != "true" ]] && eval "$v=false" || eval "$v=true"
  done
  ...
  log INFO "No config file. Using defaults (all enabled)."
...
run_cmd "grep -q pam_tid /etc/pam.d/sudo || echo -e 'auth\\tsufficient\\tpam_tid.so' | sudo tee -a /etc/pam.d/sudo"
...
if $ENABLE_YUBI; then
  ...
  if run_cmd "ssh-keygen -t ed25519-sk -f ~/.ssh/id_ed25519_sk -N \"\" -O resident -O application=ssh:$(whoami)@$(hostname) 2>/dev/null"; then
    ...
    log WARN "No FIDO2 key detected; skipping keygen"
  fi
fi
...
<dict>
  ...
  <array><string>$HOME/Scripts/battAlert.sh</string></array>
...
if [[ -x "$HOME/Tools/macDeepDive.sh" ]]; then
  ...
  echo "Error: macDeepDive.sh not found!" >&2; exit 1
fi
cd "$HOME/AuditLogs" || exit 1
[[ ! -d .git ]] && git init
...
if $INSTALL_OLLAMA; then
  ...
  run_cmd "brew services start ollama || true"; sleep 5
  log INFO "Pulling Mistral model…"; run_cmd "ollama pull mistral:7b || true"
fi

########################## 7. Advanced Networking ##############################

log INFO "Starting advanced networking setup…"

# 7a) Make sure Colima is running with --network-address
run_cmd "colima start --network-address --arch aarch64 --vm-type vz --vz-rosetta || true"

# 7b) Allow passwordless sudo for networksetup (only DNS changes)
SUDOERS_FILE="/etc/sudoers.d/nextlevel-network"
if [[ ! -f "$SUDOERS_FILE" ]]; then
  log INFO "Granting passwordless sudo for networksetup…"
  cat <<EOF | sudo tee "$SUDOERS_FILE" >/dev/null
# nextLevel2.sh: allow $USER to modify DNS without password
$USER ALL=(root) NOPASSWD: /usr/sbin/networksetup -setdnsservers *
EOF
  sudo chmod 440 "$SUDOERS_FILE"
  log INFO "✓ Passwordless sudo configured for DNS changes."
fi

# 7c) Detect primary network service on macOS
...
if [[ -z "$PRIMARY_SERVICE" ]]; then
  log ERROR "Cannot detect network service; skipping Pi-hole & DoH"
...
if [[ -z "$VM_IP" || "$VM_IP" == "null" ]]; then
  log ERROR "Could not get Colima VM IP; skipping Pi-hole & DoH"
  INSTALL_PIHOLE=false
  INSTALL_DOH=false
else
...
# 7e) Launch Cloudflared DNS-over-HTTPS on the host (so Pi-hole can use it)
if [[ "$INSTALL_DOH" == "true" ]]; then
  log INFO "Configuring Cloudflared DoH proxy…"
  if ! $DRYRUN; then
...
EOF
  ...
  run_cmd "launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/com.local.doh.plist 2>/dev/null || true"
  run_cmd "launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.local.doh.plist"
  run_cmd "launchctl enable gui/$(id -u)/com.local.doh"
  log INFO "✓ Cloudflared DoH proxy listening on $VM_IP:5053"
fi

# 7f) Deploy Pi-hole inside Colima, binding only to the VM IP and pointing it at our DoH proxy
if [[ "$INSTALL_PIHOLE" == "true" ]]; then
  log INFO "Deploying Pi-hole container (bound to $VM_IP:53)…"
  run_cmd "docker rm -f pihole 2>/dev/null || true"
  run_cmd "docker run -d --name pihole \
    -p ${VM_IP}:53:53/tcp -p ${VM_IP}:53:53/udp -p ${VM_IP}:80:80 \
    --restart unless-stopped \
    -v pihole_data:/etc/pihole \
    -e TZ=$(date +%Z) \
    -e DNS1=${VM_IP}#5053 \
    pihole/pihole:latest"
  log INFO "✓ Pi-hole deployed and pre-configured to use DoH via ${VM_IP}#5053"
fi

# 7g) Point macOS DNS at Pi-hole and flush caches
if [[ "$INSTALL_PIHOLE" == "true" ]]; then
  ...
  run_cmd "dscacheutil -flushcache && sudo killall -HUP mDNSResponder"
...
run_cmd '[[ $(sysctl -n sysctl.proc_translated) -eq 0 ]] && echo "✅ ARM-only" || echo "❌ Rosetta active"'
run_cmd 'curl -s --max-time 5 http://localhost:11434/v1/models | grep mistral && echo "✅ Ollama running" || echo "❌ Ollama down"'
run_cmd 'scutil --dns | grep nameserver | grep -E "127\.0\.0\.1(:5053)?" && echo "✅ Local DNS filtering" || echo "❌ DNS not pointing to localhost"'
run_cmd 'launchctl print gui/$(id -u)/com.local.battalert &>/dev/null && echo "✅ battAlert loaded" || echo "❌ battAlert missing"'
run_cmd 'launchctl print gui/$(id -u)/com.local.weeklyaudit &>/dev/null && echo "✅ weeklyAudit loaded" || echo "❌ weeklyAudit missing"'
$INSTALL_DOH && run_cmd 'launchctl print gui/$(id -u)/com.local.doh &>/dev/null && echo "✅ DoH Agent loaded" || echo "❌ DoH Agent missing"'
